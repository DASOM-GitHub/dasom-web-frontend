...
import { RecruitPeriodData } from './Recruittype'

const RecruitInfo: React.FC = () => {
    const [periodData, setPeriodData] = useState<RecruitPeriodData>({
    ...
    useEffect(() => {
        fetchRecruitConfigs()
            .then(data => {
                setPeriodData({
                    recruitmentPeriodStart: data.find(item => item.key === 'RECRUITMENT_PERIOD_START')?.value.substring(0, 10) || '',
                    recruitmentPeriodEnd: data.find(item => item.key === 'RECRUITMENT_PERIOD_END')?.value.substring(0, 10) || '',
                    interviewPeriodStart: data.find(item => item.key === 'INTERVIEW_PERIOD_START')?.value.substring(0, 10) || '',
                    interviewPeriodEnd: data.find(item => item.key === 'INTERVIEW_PERIOD_END')?.value.substring(0, 10) || '',
                    interviewPassAnnouncement: data.find(item => item.key === 'INTERVIEW_PASS_ANNOUNCEMENT')?.value.substring(0, 10) || ''
                })
            })
            .catch(error => {
    ...
    return(
        <div className='flex flex-col items-center w-full max-w-full min-h-screen overflow-x-hidden bg-mainBlack text-white pb-20'>
            ...
            <RecruitInfo_Button text='34기 지원하기' onClick={() => navigate('/recruit')}/>
            ...
            <div className='flex flex-col mt-96 w-[90%] items-center font-pretendardRegular overflow-x-hidden'>
                ...
                <div className='my-10'>
                    {periodData.recruitmentPeriodStart && periodData.recruitmentPeriodEnd ? (
                        <RecruitCalendar
                            periods={[{
                                start: periodData.recruitmentPeriodStart,
                                end: periodData.recruitmentPeriodEnd,
                            }]}
                            interviewPeriods={periodData.interviewPeriodStart && periodData.interviewPeriodEnd ? [
                                {
                                    start: periodData.interviewPeriodStart,
                                    end: periodData.interviewPeriodEnd,
                                },
                            ] : []}
                            passDates={periodData.interviewPassAnnouncement ? [periodData.interviewPassAnnouncement] : []}
                            colors={{
                ...
                <div className='flex flex-col items-center text-lg gap-2'>
                    <p><span className='font-pretendardBold'>1. 서류모집</span> {formatMmDd(periodData.recruitmentPeriodStart)} ~ {formatMmDd(periodData.recruitmentPeriodEnd)}</p>
                    <p><span className='font-pretendardBold'>2. 면접일정</span> {formatMmDd(periodData.interviewPeriodStart)} ~ {formatMmDd(periodData.interviewPeriodEnd)}</p>
                    <p><span className='font-pretendardBold'>3. 최종 합격자 발표</span> {formatMmDd(periodData.interviewPassAnnouncement)}</p>
                    <p><span className='font-pretendardBold'>4. 다솜 2학기 OT</span> 09월 19일</p>
                </div>
                <p className='mt-6 text-base text-subGrey2'>일정은 매 기수 변동될 수 있습니다.</p>
            </div>
